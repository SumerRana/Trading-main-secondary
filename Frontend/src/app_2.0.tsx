...
const App: React.FC = () => {
  ...
  const [tempTokenValue, setTempTokenValue] = useState([0,0,0,0,0]);
  ...
  const addValueTOToken = () => {
    for (let i = 0; i < tokensOffered.length; i++) {
    if (tokensOffered[i].token === "WOOD") {
      setTempTokenValue[0] = tokensOffered[i].amount;
    } else if (tokensOffered[i].token === "ROCK") {
      setTempTokenValue[1] = tokensOffered[i].amount;
    } else if (tokensOffered[i].token === "CLAY") {
      setTempTokenValue[2] = tokensOffered[i].amount;
    } else if (tokensOffered[i].token === "WOOL") {
      setTempTokenValue[3] = tokensOffered[i].amount;
    }
    }
  ...
  const getWoodAllowance = async () => {
    console.log(tempTokenValue);
    if (web3 && account && chainId && tempTokenValue[0] > 0) {
      console.log(tempTokenValue);
  ...
  const getRockAllowance = async () => {
    if (web3 && account && chainId && tempTokenValue[1] > 0) {
  ...
  const getClayAllowance = async () => {
    if (web3 && account && chainId && tempTokenValue[2] > 0) {
  ...
  const getWoolAllowance = async () => {
    if (web3 && account && chainId && tempTokenValue[3] > 0) {
  ...
  const getFishAllowance = async () => {
    if (web3 && account && chainId && tempTokenValue[4] > 0) {
